        ...
        <Card>
          <ActionForm action={updateSettings} className="flex flex-col gap-5">
            ...
            <label className="flex flex-col gap-2">
              ...
              <select name="plan" defaultValue={workspace.plan} className={FIELD}>
                {Object.values(PLANS).map((entry) => (
                  <option key={entry.id} value={entry.id}>
                    {entry.name} — {entry.monthlyCredits} crédits/mois
                  </option>
                ))}
              </select>
              <span className="text-[12.5px] text-muted">
                En production, ce champ est piloté par Paddle ; il est modifiable ici pour
                exercer les quotas.
              </span>
            </label>
        ...
        <div className="flex flex-col gap-5">
          <Card>
            ...
            <dl className="flex flex-col gap-2.5 text-[13px]">
              <Row label="Identité" value={`${user.name} (@${user.handle})`} />
          ...
          <Card>
            ...
            <dl className="flex flex-col gap-2.5 text-[13px]">
              <Row
                label="Publication X"
                value={xConfig().configured ? "Connectée" : "Simulée"}
              />
